...
public class AnswerMsnSumStatController {
    ...
    public Map<String, Object> searchAnswerMsn(@PathVariable(required = false,value = "pageNumber") Integer pageNumber, Model model, @RequestBody AnswerMsnSumStatSearchDto dto, HttpSession session){
        ...
        int totalLandingCount =  limitedAnswerMsns.stream().mapToInt(AnswerMsnSumStat::getLandingCnt).sum();  // 랜딩카운트 합
        int totalPartCount =   limitedAnswerMsns.stream().mapToInt(AnswerMsnSumStat::getPartCnt).sum();  // 참여카운트 합
