...
public class AnswerMsnController {
    ...
    @PostMapping({"/Mission/quizList/search","/Mission/quizList/search/","/Mission/quizList/search/{pageNumber}"})
    @ResponseBody
    @ApiResponses(value = {
    ...
    @GetMapping("/Mission/quizList/search/{pageNumber}")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "검색 완료(조건에 맞는결과가없을경우 빈 리스트 반환)"),
    })
    ...
    @Operation(summary = "정답미션 검색", description = "조건에 맞는 정답미션을 검색합니다")
    @PostMapping({"/Mission/quizCurrentList/search","/Mission/quizCurrentList/search/","/Mission/quizCurrentList/search/{pageNumber}"})
    @ResponseBody
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "검색 완료(조건에 맞는결과가없을경우 빈 리스트 반환)"),
            @ApiResponse(responseCode = "401", description = "세션이 없거나 만료됨"),
    ...
    @GetMapping("/Mission/quizCurrentList/search/{pageNumber}")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "검색 완료(조건에 맞는결과가없을경우 빈 리스트 반환)"),
    })
    public String  searchCurrentAnswerMsn_return(@PathVariable(required = false,value = "pageNumber") Integer pageNumber,HttpSession session,Model model){
        Member sessionMember = (Member) session.getAttribute("member");
        List<Advertiser> advertisers = advertiserService.getAdvertisers();
        List<Server> servers = serverService.getServers();
        if (sessionMember == null) {
    ...
    public ResponseEntity<Void> reportExcelDownload(@PathVariable(required = true,value = "idx")int idx,HttpSession session,HttpServletResponse response)throws IOException {
        Member sessionMember= (Member) session.getAttribute("member");
        if(sessionMember == null){
    ...
    @GetMapping("/Mission/quizExcelForm")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "401", description = "세션이 없거나 만료됨"),
            @ApiResponse(responseCode = "403", description = "권한없음"),
    ...
    public ResponseEntity<Void> excelFormDownload(HttpSession session,HttpServletResponse response)throws IOException {
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            ...
            Sheet sheet = answerMsnService.downloadQuizForm(wb);
            if(sheet !=null) {
                String fileName = "정답 미션 리스트.xlsx";
                fileName = URLEncoder.encode(fileName, StandardCharsets.UTF_8);
                response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
                response.setHeader("Content-Disposition", "attachment;filename=\"" + fileName + "\"");
                wb.write(response.getOutputStream());
